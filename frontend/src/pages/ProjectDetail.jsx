...
import axios from 'axios'; // Ensure this is either the default import or from your config file
...
// Set Axios base URL (alternatively, use Vite proxy)
    ...
    const fetchProjectData = async () => {
      try {
        ...
        if (!projectData.status) {
          console.warn('Project status missing, defaulting to "pending"');
          projectData.status = 'pending';
        }
        setProject(projectData);
        
        const clientRes = await axios.get(`/api/clients/${projectData.client_id}`);
        setClient(clientRes.data);
        
        const timeRes = await axios.get(`/api/time/?project_id=${id}`);
        setTimeEntries(Array.isArray(timeRes.data) ? timeRes.data : []);
        
        const invoicesRes = await axios.get(`/api/invoices/?project_id=${id}`);
        setInvoices(Array.isArray(invoicesRes.data) ? invoicesRes.data : []);
        
        const documentsRes = await axios.get(`/api/documents/?project_id=${id}`);
                      ...
                      <div className="flex items-start">
                        ...
                        <div className="ml-4">
                          ...
                          <p className="text-xs text-gray-500">
                            {doc.document_type ? doc.document_type.charAt(0).toUpperCase() + doc.document_type.slice(1) : 'Unknown'}
                          </p>
